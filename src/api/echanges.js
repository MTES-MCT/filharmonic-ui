...
export default class EchangesAPI extends BaseAPI {
  async list (options = {}) {
    let filteredEchanges = _.cloneDeep(echanges)
    if (options.filter) {
      filteredEchanges = filteredEchanges.filter(options.filter)
    }
    return Promise.all(
      filteredEchanges.map(async echange => {
        if (options.inspectionId) {
          echange.inspection = await this.api.inspections.get(echange.inspectionId)
        }
  ...
  async save (newEchange) {
    ...
    console.log('newEchange=' + JSON.stringify(newEchange))
    const oldEchange = echanges.find(e => e.id === newEchange.id)
    console.log('oldEchange=' + JSON.stringify(oldEchange))
    Object.assign(oldEchange, _.cloneDeep(newEchange))
    console.log('echanges=' + JSON.stringify(echanges))
  }
  async get (id, options) {
    ...
    if (!echange) {
      throw new ApplicationError(`Echange ${id} non trouvée`)
    }
    return echange
  }
}
