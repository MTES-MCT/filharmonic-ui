...
// extraire la vue générée de l'API (séparer backend API de la présentation frontend)
...
<br>
Affaire suivie par : ${user.name}<br>
Cellule TESSP<br>
...
`
  ...
  _genererFooter (etablissement, user) {
    return `
...
`
  ...
  genererLettreAnnonce (detail, etablissement, echanges, user) {
    const pointsDeControles = echanges.map(echange => {
      return ` - <strong>${echange.sujet}</strong> (${echange.referencesReglementaires.join(', ')})<br>`
    }).join('')
    return `
<div style="text-align: justify">
${this._genererHeader(user)}
<br>
...
<br>
Je vous informe de mon intention de procéder à une visite d’inspection de votre installation « ${etablissement.nom} »
situé ${etablissement.adresse} le <strong>${new Date(detail.date).toLocaleDateString()}</strong> à partir de <strong>9h30</strong>.<br>
<br>
...
<br>
${this._genererFooter(etablissement, user)}
</div>
`
  }

  genererLettreSuites (detail, etablissement, user) {
    return `
<div style="text-align: justify">
${this._genererHeader(user)}
<br>
<strong>Objet :</strong> Visite d'inspection du ${new Date(detail.date).toLocaleDateString()}<br>
<strong>P.J. :</strong> Copie du rapport d'inspection<br>
<br>
Monsieur le directeur,<br>
<br>
L'inspection des installations classées a réalisé une visite d'inspection de votre établissement situé ${etablissement.adresse}, le ${new Date(detail.date).toLocaleDateString()}.<br>
<br>
...
<br>
Cette visite d'detail a mis en exergue des non-conformités détaillées dans le rapport joint. Je vous saurais gré
